...
import com.lfork.phonelimitadvanced.limit.PhoneLimitController.AUTO_UNLOCKED
import com.lfork.phonelimitadvanced.limit.PhoneLimitController.FORCE_UNLOCKED
...
class LimitService : Service() {
    ...
    override fun onStartCommand(intent: Intent, flags: Int, startId: Int): Int {
        ...
        if (PhoneLimitController.startLimit(limitTime)) {
            showNotification()
...
//        WIFIUtil(applicationContext).closeWifi()
        Thread(Runnable {

            val unlockType = PhoneLimitController.startAutoUnlock()
    ...
    private fun startStateCheck() {
        Thread(Runnable {
            var remainTime = PhoneLimitController.getRemainTimeSeconds()
            Log.d("timeTest", "开始状态刷新,剩余时间${remainTime}秒")
            while (remainTime > 0) {

                listener?.remainTimeRefreshed(remainTime)
                Thread.sleep(999)
                remainTime = PhoneLimitController.getRemainTimeSeconds()
            }
        }).start()
    }
